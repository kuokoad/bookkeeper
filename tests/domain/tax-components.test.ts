...
/**
 * The two loops below walk every price from -50.00 to 50.00 for each rate set:
 * 10,001 cases apiece. That is the point — a tax identity that holds for chosen
 * examples and fails at 3,847 pesewas is exactly the bug this suite exists to
 * catch, so the loop stays exhaustive rather than sampled. It is slow by
 * design; see the timeout note in vitest.config.ts.
 */
